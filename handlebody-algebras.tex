...
more general 3-manifolds brings the topology of the ambient space into the play.
In all the classical examples, like the Temperley--Lieb algebra or 
the Hecke algebra, the ambient space is the 3-ball, and these algebras 
are related to spherical Coxeter and Artin--Tits braid groups.
In the simplest case beyond 3-ball, when we pass to links in a solid torus, 
these diagrams algebras get replaced by their (extended) affine version, 
...
one could expect for more general 3-manifolds. Most prominently 
for this paper, (3-)handlebodies of genus $g$; the two cases mentioned 
before are $g=0$ and $g=1$, respectively.

The case $g=0$ is classical, and these diagram algebras and their associated 
braid groups are around for Donkey's years. The $g=1$ case goes 
back to at least Brieskorn \cite{Brieskorn}, and has a long history of work on this topic.
For example (with more references to come in the main text), see
...
while Lambropoulou studied links in handlebodies even before that \cite{La-PhDThesis}.

From the representation theoretical point of view all these 
algebras share the common feature of being cellular in a certain sense 
that we will make precise. In a nutshell, in the $g=0$ case 
these algebras are cellular in the sense of Graham--Lehrer \cite{GrLe-cellular}, 
in the $g=1$ case they are affine cellular in the sense of K{\"o}nig--Xi \cite{KoXi-affine-cellular}.
For $g>1$ we explain how they are sandwich cellular, a notion that 
generalizes (affine) cellularity. 
Note hereby that this generalization is strictly speaking not new, 
it heavily builds on and borrows from \cite{Gr-semigroups}, \cite{KoXi-cellular-inflation-morita} or \cite{GuWi-almost-cellular}, 
but our exposition appears to be new.

We see this paper as a continuation of these works, focusing on the diagrammatic 
and representation theoretical aspects.

\subsection{What this paper does}

\DT{Rework!}

In this paper we concentrate on various algebraic structures controling the extension to handlebodies of several classical link invariants.

Our notion of cellularity recollects \PV{...} and it is the one that best fits all our examples (and many more) is \PV{sandwich. Not completely new...}
In~\cite{ElLa-trace-hecke} (not in Libedinsky(light leaves)?) the authors have introduced the notion of object-adapted cellularity in order to study a cellular structure in the hom algebras ot the Soergel category. \PV{cite~\cite{EhTu-relcell} and/or \cite{We-tensors-cellular-categories} here? }



\medskip

\fullref{section:cells}:
A generalization of cellularity



\PV{Maybe add some relations between these diagrams}


\fullref{section:braids}: Handlebody braid and Coxeter groups
...
\fullref{section:tlblob}: Handlebody Temperley--Lieb and blob algebras
\[
\begin{tikzpicture}[anchorbase,scale=0.75,tinynodes]
\draw[pole,crosspole] (-0.5,0) to[out=90,in=270] (-0.5,1.5);
...
\fullref{section:brauer}: Handlebody Brauer and BMW algebras
...
\fullref{section:hecke}: Handlebody Hecke and Ariki--Koike algebras
...
\paragraph{Temperley--Lieb is important} The reader should read~\fullref{section:tlblob} before BMW~\fullref{section:brauer} or Hecke~\fullref{section:hecke}.
...
\begin{remark}
Let us comment on a few potential generalizations.\DT{To do -- cite people}
\end{remark}

\PV{3 items: 1 - other cellular algebras, 2 - categorification, 3 - connections to Lie theory (Schur-Weyl)}

Diagrammatic algebras are also important in categorification. 
After its introduction by Khovanov and Lauda \cite{KL1} they have become quite popular. \PV{cite Webster papers\cite{webster}, 2-heisenberg papers, 2-Verma \cite{naissevaz2,naissevaz3,maksimau-vaz} and 2-blob \cite{lacabanne-naisse-vaz}, and Elias--Williamson}.  
Speculate that the algebras in the papers cited above should fall in our notion of cellularity.
\PV{Cite Kleshchev et al's papers on affine cellularity of KLR here?}

Mention ``hypothetical'' handlebody Soergel bimodules and~\cite{RoTu-homflypt-typea}, APS + annular Khovanov, 2-blob, etc... 

What about connections to RTW or Lie algebras via Schur-Weyl like in LV\cite{LV}?

\subsection{How to read this paper}

The second section \fullref{section:cells} explains our generalization 
of cellularity and is independent of the rest. It can be easily skipped 
during a first reading.
The third section \fullref{section:braids} is fundamental for 
...
to the diagrammatics of handlebodies. We also thank zoom -- you deserve it!
...
\emph{handlebody braid diagrams} (of genus $g$ and 
$n$ strands).
...
sometimes say \emph{twists} for short.
...
For $i=1$ we let $\jm_{u,i}^{\pm 1}=\tau_{u}^{\pm 1}$.
...
such that $\jm_{u,i}^{\pm}$ appears right/above
...
$\cpar=(\cvar_{\gamma})\in\KK$, one for each $\setstuff{F}_{g}$-colored circle $\gamma$, up to conjugacy of words.
...
\item We have $\setstuff{L}_{0,n}=\K$, so the above 
...
\begin{remark}
Imposing a single relation implying only $\tau_{g}$ (this is the first twist counting from the right, {\cf} \eqref{eq:lefttoright-labels}) gives a handlebody version of Ariki--Koike algebras.
...
affine version Ariki--Koike algebras.
\end{remark}
...
\newblock \href{https://doi.org/10.1090/S0002-9947-02-02944-6}
...
\href{https://doi.org/10.1515/crll.1997.482.191}{doi:10.1515/crll.1997.482.191}.
...
\href{https://doi.org/10.1090/ert/568}{doi:10.1090/ert/568}.
...
{\path{doi.org/10.1090/S1088-4165-09-00346-X}}.
...
\newblock \href
{https://doi.org/10.1006/jabr.1995.1079} {\path{10.1006/jabr.1995.1079}}.
...
{\path {doi.org/10.1007/s00029-018-0397-z}}.
