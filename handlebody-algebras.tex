...
\lambda_{u_{m},i_{m}}^{a_{m}}$ with the 
...
\begin{proof}
To show that \eqref{eq:hecke-basis} spans
...
the $D$ to the left, that is, $\lambda_{u_{1}^{},i_{1}}^{a_{1}}\dots 
...
which shows that $c_{D,b,U}^{\lambda}$ span. 
...
$\bsym{i}$, but the property $i_{1}\leq\dots\leq i_{m}$. In particular, 
...
word in $b_{v}$ for $v\neq u$. 
\end{definition}
The relations imply that no strand can carry more than $d_{u}$ blobs of label $u$.
