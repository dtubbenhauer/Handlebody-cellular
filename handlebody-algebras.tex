...
more general three manifolds brings the topology of the ambient space into the play.
...
For example (with more references to come in the main text after the introduction), see
...
algebraic and representation theoretical aspects.
...
The three strands to the right are usual strands.
The four thick and blue/grayish strands to the left are core
strands and they correspond to the punctures 
of the disk respectively the cores of the handlebody. 
The point is that via an appropriate closure, merging 
...
handlebody, hence the name. All links in such handlebodies can 
...
These handlebody braid pictures our starting point to define and study 
various diagram algebras associated to handlebodies:

\begin{enumerate}

\item In \fullref{section:tlblob} we study handlebody Temperley--Lieb and blob algebras
where the pictures to keep in mind are 
(left Temperley--Lieb, right blob):
\begin{gather*}
\begin{tikzpicture}[anchorbase,scale=0.75,tinynodes]
\draw[pole,crosspole] (-0.5,0) to[out=90,in=270] (-0.5,1.5);
...
For $g=1$ they are not the same anymore (at least in our formulation as they appear 
under various names in the literature), and have a long history of study
starting with \cite{MaSa-blob}.

\item In \fullref{section:brauer} we move on to handlebody versions 
of Brauer and BMW algebras. These are handlebody tangle algebras 
and the picture is
\begin{gather*}
\begin{tikzpicture}[anchorbase,scale=0.75,tinynodes]
...
.
\end{gather*}
The cases $g=0$ and $g=1$ are, of course, well-studied as the correspond to 
Hecke respectively extended affine Hecke algebras or their cyclotomic quotients, and
...
\fullref{section:cells}, which gives us a tool to parameterize the simples 
...
this work would be to study these above algebras 
in connections to quantum topology and its ramifications.

For example, \cite{Geck-Lambropoulou} and \cite{OrRa-affine-braids}
study Markov traces for $g=1$ and construct link invariants, and
these link invariants admit categorifications \cite{WeWi}. 
For $g>1$ steps in this direction were taken in \cite{RoTu-homflypt-typea}, 
but this direction appears to be widely open otherwise.

Moreover, for $g=0$ most of these diagram algebras are related to 
representation theory by some version of Schur--Weyl duality. 
(In fact, this was the reason to define the Temperley--Lieb 
algebras to begin with, see \cite{RuTeWe}.) 
Some work for higher genus in this direction are known, 
{\eg} in relation to Verma modules \cite{IoLeZh-verma-schur-weyl}, 
\cite{LV}, \cite{DaRa-two-boundary-hecke} or complex reflection groups \cite{MaSt}, \cite{SaSh}. Future research in this direction seems 
to be a worthwhile goal.

\item \textbf{Diagram algebras.} There are plenty 
of diagram algebras which we have not considered in this 
...
After its introduction by Khovanov--Lauda \cite{KL1} 
(and Rouquier \cite{Rouquier}) 
they have become quite popular, and might admit handlebody versions.
For example, these KLR algebras come to mind, 
or Webster's tensor product algebras \cite{webster}, 
algebras related to Verma categorification 
...
All of our objects under study also have appropriate 
categorical versions.

For example, it should be fairly straightforward 
to generalize our discussion of \fullref{section:cells} 
to cellular categories as studied in \cite{We-tensors-cellular-categories} or
\cite{ElLa-trace-hecke}.

However, let us mention that one reason why we have not 
